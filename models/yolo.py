...
from models.MobileViTmodels import MobileViTBlock
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, Decoupled_Detect, Detect):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Decoupled_Detect)):
            ...
            self.model.to(torch.device('cuda')) #new added line and below 3
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, C3_DCNV3,
                 DCNV3_YoLo):
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, C3x, C3_DCNV3, DCNV3_YoLo]:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m is nn.BatchNorm2d:
            args = [ch[f]]
        # for attention with channels
        elif m in [TripletAttention, MobileViTBlock]:
            args = [ch[f], *args]
        elif m is Concat:
